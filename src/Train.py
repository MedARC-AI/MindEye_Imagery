...
from models import IPAdapterPlusXL_LoRA, IPAdapterPlusXLEmbedder, IPAdapterXLEmbedder, IPAdapterXL_LoRA
...
if utils.is_interactive(): # set batch size here if using interactive notebook instead of submitting job
    global_batch_size = batch_size = 500
else:
...
if utils.is_interactive():
    model_name = "ip_adapt_finetune_subj1"
    ...
    #                 --ckpt_interval=5 --ckpt_saving --wandb_log"
    ...
    jupyter_args = f"--data_path=/weka/proj-medarc/shared/mindeyev2_dataset \
                    ...
                    --use_prior --prior_scale=30 \
                    --n_blocks=4 --max_lr=3e-4 --mixup_pct=.33 --num_epochs=150 --no-use_image_aug \
                    --ckpt_interval=5 --ckpt_saving --wandb_log --multisubject_ckpt=../train_logs/ip_adapt_multi_exl1_new"
...
parser.add_argument(
    "--plus",action=argparse.BooleanOptionalAction,default=False,
    help="Use the IP Adapter Plus model for SDXL",
)
...
if plus:
    ...
    ip_img_embedder = IPAdapterPlusXLEmbedder(image_encoder_path, ip_ckpt, device, num_tokens=16)
    clip_seq_dim = 16
else:
    ip_ckpt = "IP-Adapter/sdxl_models/ip-adapter_sdxl_vit-h.safetensors"
    ip_img_embedder = IPAdapterXLEmbedder(image_encoder_path, ip_ckpt, device, num_tokens=4)
    clip_seq_dim = 4
...
if blurry_recon:
    ...
    autoenc = AutoencoderKL.from_pretrained("madebyollin/sdxl-vae-fp16-fix", torch_dtype=torch.float16)  
...
for epoch in progress_bar:
    ...
    for train_i in range(num_iterations_per_epoch):
        with torch.cuda.amp.autocast(dtype=data_type):
            ...
            # This conversion to PIL images and back is clunky, there should be a way to avoid it
            image_list = []
            for ind_image in image:
                image_list.append(transforms.ToPILImage()(ind_image.cpu()).convert('RGB'))
            clip_target = ip_img_embedder.get_image_embeds(image_list).to(device)
