...
class IPAdapter:
    def __init__(self, sd_pipe, image_encoder_path, ip_ckpt, device, num_tokens=4, lora=True):
        self.device = device
        ...
        if lora:
            ...
            self.pipe.load_lora_weights("latent-consistency/lcm-lora-sdxl", torch_dtype=torch.float16)
            self.pipe.fuse_lora()
...
class IPAdapterPlusXL_LoRA(IPAdapter):
    """SDXL"""
