...
class SC_Reconstructor(object):
    ...
    def reconstruct(self,
        ...
        if int(strength * num_steps_c) > 0:
            ...
            if c_i is None and c_t is not None:
                textstrength=1
                c_i = torch.zeros((n_samples, 1, 768), device=self.device, dtype=self.dtype)
            elif c_i is not None and c_t is None:
                textstrength=0
                c_t = torch.zeros((n_samples, 77, 1280), device=self.device, dtype=self.dtype)
