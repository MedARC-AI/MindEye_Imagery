...
def condition_average(x, y, cond, nest=False, num_trial_reps=1000):
    idx, idx_count = np.unique(cond, return_counts=True)
    num_trial_reps = min(num_trial_reps, idx_count.max())
    idx_list = [np.array(cond)==i for i in np.sort(idx)]
    if nest:
        avg_x = torch.zeros((len(idx), num_trial_reps, x.shape[1]), dtype=torch.float32)
    else:
    ...
    for i, m in enumerate(idx_list):
        num_trial_reps = min(num_trial_reps, len(m))
        if nest:
            if np.sum(m) == num_trial_reps:
                avg_x[i] = x[m]
            else:
                avg_x[i,:max(np.sum(m), num_trial_reps)] = x[m][:num_trial_reps]
        else:
            avg_x[i] = torch.mean(x[m][:num_trial_reps], axis=0)
        arranged_y[i] = y[m[0]]
...
def load_nsd_mental_imagery(subject, mode, stimtype="all", average=False, num_trial_reps = 16, nest=False, data_root="../dataset/"):
    ...
    if average or nest:
        x, y, sample_count = condition_average(x, y, conditionals, nest=nest, num_trial_reps=num_trial_reps)
    else:
