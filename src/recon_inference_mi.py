...
parser.add_argument(
    "--filter_contrast",action=argparse.BooleanOptionalAction, default=False,
    help="Filter the low level output to be more intense and smoothed",
)
parser.add_argument(
    "--filter_sharpness",action=argparse.BooleanOptionalAction, default=False,
    help="Filter the low level output to be more intense and smoothed",
)
parser.add_argument(
...
for idx in tqdm(range(0,voxels.shape[0]), desc="sample loop"):
    ...
    if blurry_recon:
        if vdvae:
            blurred_image = Image.open(f"{raw_path}/{mode}/braindiffuser/subject{subj}/{idx}/low_level.png")
            # latent_voxels = reconstructor.embed_latent(blurred_image)
        else:
            latent_voxels = pred_blurry_vae[idx]
            blurred_image = reconstructor.reconstruct(latent=latent_voxels, strength=0.0)
            blurred_image = transforms.ToPILImage()(blurred_image[0])
        if filter_sharpness:
        ...
        if filter_color: 
            blurred_image = ImageEnhance.Color(blurred_image).enhance(0.7)
