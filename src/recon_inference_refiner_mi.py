...
base = DiffusionPipeline.from_pretrained(
    "stabilityai/stable-diffusion-xl-base-1.0", torch_dtype=torch.float16, variant="fp16", use_safetensors=True, cache_dir = cache_dir
)
base.to("cuda")
...
unclip_path = "/weka/proj-fmri/jonxu/MindEye_Imagery/train_logs/jonathan_unclip"

pred_clip_image = torch.zeros((len(all_images), clip_seq_dim, clip_emb_dim)).to("cpu")
with open(f'{unclip_path}/ridge_image_weights.pkl', 'rb') as f:
...
if dual_guidance:
    with open(f'{unclip_path}/ridge_text_weights.pkl', 'rb') as f:
...
if prompt_recon:
    with open(f'{unclip_path}/ridge_prompt_weights.pkl', 'rb') as f:
...
if blurry_recon:
    ...
    with open(f'{unclip_path}/ridge_blurry_weights.pkl', 'rb') as f:
...
if retrieval:
    ...
    with open(f'{unclip_path}/ridge_retrieval_weights.pkl', 'rb') as f:
...
if prompt_recon:
    pred_caption_path = f"evals/{model_name}/{model_name}_all_predcaptions_{mode}.pt"
    if os.path.exists(pred_caption_path):
        all_predcaptions = torch.load(pred_caption_path)
    else:
        ...
        torch.save(all_predcaptions,pred_caption_path)
...
if num_images_per_sample == 1:
    ...
    for rep in tqdm(range(gen_rep)):
        ...
        for idx in tqdm(range(0,voxels.shape[0]), desc="sample loop"):
            ...
            samples_multi = torch.load(f"evals/jonathan_unclip/samples/{mode}/{rep}/{idx}.pt").to(device)

            # Refiner step
            captions = [all_predcaptions.tolist()[idx]]*gen_rep
            refined_samples_multi = refiner(
                prompt=captions,
                num_inference_steps=n_steps,
                denoising_start=high_noise_frac,
                image=samples_multi,
            ).images
            # print(refined_samples_multi[0].shape)
            to_tensor = transforms.ToTensor()
            refined_tensors = [to_tensor(img) for img in refined_samples_multi]
            stacked_tensor = torch.stack(refined_tensors).to(device)
                        
            samples = utils.pick_best_recon(stacked_tensor, retrieval_voxels, reconstructor, hidden=retrieval).unsqueeze(0)
